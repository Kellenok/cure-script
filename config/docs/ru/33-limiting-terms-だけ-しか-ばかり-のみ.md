...
И <code>しか... ない</code> также может использоваться в таких обстоятельствах, как <code>にげるしかない</code>. <code>[逃]{に}げる</code> означает <code>убегать</code> или <code>спасаться</code>; если мы говорим <code>にげるしかない</code>, мы говорим: <code>Ничего не остаётся, кроме как бежать / Нет другого выхода, кроме как бежать</code>.
